...
(function() {
  ...
  *****************************************************************************/
  ...
  var paintitblackWidget = widgets.Widget({
    ...
    onClick: function onWidgetClick() {
      loggit("");
      loggit("");
      loggit("MAIN onWidgetClick             : %s", truncateUrl(tabs.activeTab.url) );
  ...
  function UpdateDocumentState(documentState,documentUrl) {
    ...
    if (arguments.length < 2) {
      loggit("MAIN UpdateDocumentState       : no arguments; querying active tab");
    ...
    if (documentUrl != tabs.activeTab.url) {
      loggit("MAIN UpdateDocumentState       : documentUrl is not active tab");
      loggit("                               : documentUrl  = %s", documentUrl);
      loggit("                               : activeTabUrl = %s", tabs.activeTab.url);
      return;
    }
  ...
  *****************************************************************************/
  ...
  prefSet.on("", function onPrefChange(prefName) {
    loggit("The %s preference changed.", prefName);
  ...
  *****************************************************************************/
  function SendMessageToTab(tab,message,payload) {
    try {
//    loggit("MAIN enter SendMessageToTab    ( %s , %s , %s )" , truncateUrl(tab.url), message, payload);
      loggit("MAIN SendMessageToTab          ( %s , %s , %s )" , truncateUrl(tab.url), message, payload);
      tab.worker.port.emit(message,payload);
//    loggit("MAIN leave SendMessageToTab      ---> OKAY");
      return true;
    }
    catch (e) {
//    loggit("MAIN leave SendMessageToTab      ---> ERROR = %s : %s" , e.name, e.message);
      return false;
    }
  }
  ...
  *****************************************************************************/
  (function() {
    ...
    function HandleChangedTabContent(tab) {
      loggit("MAIN enter HandleChangedTabContent : %s", truncateUrl(tab.url));

      // This message will indicate whether the worker is present and still valid.
      if ( SendMessageToTab(tab,"testValid") ) {
        loggit("MAIN                                 ---> existing worker is still valid");
        loggit("MAIN leave HandleChangedTabContent : %s", truncateUrl(tab.url));
        return;
      }

      // Destroy any previous worker.
      if (tab.hasOwnProperty("worker")) {
        loggit("MAIN                                 ---> destroying previous worker");
        tab.worker.destroy();
        tab.worker = null;
      }
      ...
      tab.worker.port.on("UpdateDocumentState", function onUpdateDocumentState(documentUrl, documentState) {
        loggit("MAIN         <<< UpdateDocumentState   ( %s , %s )", truncateUrl(documentUrl), documentState);
    ...
    tabs.on("ready", function onTabReady(tab) {
      loggit("");
      loggit("MAIN enter tabs.on('ready')    : %s", truncateUrl(tab.url));
      ...
      QueryDocumentState(tab);      //
      loggit("MAIN leave tabs.on('ready')    : %s", truncateUrl(tab.url));
      loggit("");
    });
    ...
    tabs.on("load", function onTabLoad(tab) {
      loggit("");
      loggit("MAIN enter tabs.on('load')     : %s", truncateUrl(tab.url));
      UpdatePreferences(tab);       // Make sure this tab has a copy of the current user preferences.
      QueryDocumentState(tab);
      loggit("MAIN leave tabs.on('load')     : %s", truncateUrl(tab.url));
      loggit("");
    });
    ...
    tabs.on("pageshow", function onTabPageshow(tab) {
      loggit("");
      loggit("MAIN enter tabs.on('pageshow') : %s", truncateUrl(tab.url));
      loggit("  prefSet.autoColor = %s", prefSet.prefs["autoColor"]);
      UpdatePreferences(tab);       // Make sure this tab has a copy of the current user preferences.
      ModifyDocumentColors(tab,forceUpdateColors=false);
      QueryDocumentState(tab);
      loggit("MAIN leave tabs.on('pageshow') : %s", truncateUrl(tab.url));
      loggit("");
    });
    ...
    tabs.on("activate", function onTabActivate(tab) {
      loggit("");
      loggit("MAIN enter tabs.on('activate') : %s", truncateUrl(tab.url));
      loggit("  prefSet.autoColor = %s", prefSet.prefs["autoColor"]);
      UpdatePreferences(tab);       // Make sure this tab has a copy of the current user preferences.
      ModifyDocumentColors(tab,forceUpdateColors=false);
      QueryDocumentState(tab);
      loggit("MAIN leave tabs.on('activate') : %s", truncateUrl(tab.url));
      loggit("");
    });
    ...
    tabs.on("deactivate", function onTabDeactivate(tab) {
      loggit("");
      loggit("MAIN tabs.on('deactivate')     : %s", truncateUrl(tab.url));
    });
    ...
    tabs.on("close", function onTabClose(tab) {
      loggit("");
      loggit("MAIN tabs.on('close')          : %s", truncateUrl(tab.url));
    });
