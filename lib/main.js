...
(function() {
  ...
  *****************************************************************************/
  ...
  //--------------------------------------------------
  const sprintf = require("./3rdParty/sprintf").sprintf;
  ...
  *****************************************************************************/
  (function() {
    ...
    function HandleChangedTabContent(tab) {
      ...
      tab.worker = tab.attach({
        contentScriptFile: [
          ...
          data.url("3rdParty/tinycolor.js"),
          data.url("../lib/3rdParty/sprintf.js"),   // under '../lib' so it can be accessed by main and by content script
    ...
    tabs.on("ready", function onTabReady(tab) {
      ...
      UpdatePreferences(tab);       // Make sure this tab has a copy of the
    ...
    tabs.on("pageshow", function onTabPageshow(tab) {
      QueryDocumentState(tab);      // The following steps may take a while
                                    // when multiple tabs are loading in the
                                    // background, so make sure the widget is
                                    // diplaying correctly.
      UpdatePreferences(tab);       // Make sure this tab has a copy of the
                                    //current user preferences.
    ...
    //    - AutoColor may currently be turned off, but the page could still be
